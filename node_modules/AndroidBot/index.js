...
class AndroidBot {
    ...
    */
    ...
    /**
     * 多次返回
     * @param {int} count 返回次数
     */
    async manyBack(count, sCount) {
        if (!count) {
            count = 1;
        }
        let strData = this.setSendData("back");
        let maxCount = count + 3;
        while (count > 0 && maxCount > 0) {
            let byteRet = await this.sendData(strData);
            if (byteRet.toString() != "false") {
                count--;
            }
            maxCount--;
            await this.sleep(sCount || 500);
        }
        return maxCount == 3;
    }
    ...
    */
    ocr = async (left, top, right, bottom, scale) => {
        let strData = this.setSendData("ocr", left, top, right, bottom, scale);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
    ...
    */
    async getWords(options = {}) {
        ...
        let wordsResult = await this.ocr(left, top, right, bottom, scale);
        if (wordsResult == null)
            return null;
    ...
    */
    async showToast(text) {
        let strData = this.setSendData("showToast", text);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if (strRet == "false")
            return false;
    ...
    */
    async openUri(uri) {
        let strData = this.setSendData("openUri", uri);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if (strRet == "false")
            return false;
